...
public class GDriveResource {
	...
	private static final String USER_TOKEN = "mydms.user.gdrive";
	...
	public SimpleResult unlinkAccount() {
		...
		try {
			...
			throw new WebApplicationException(String.format("Could not unlink account: %s!", e.getMessage()), Response.Status.BAD_GATEWAY);
		}
		return result;
	}
	...
	public Response getFile(@QueryParam("path") String path) {
		...
		try {
			...
			Optional<GDriveFile> filePayload = this.client.getFile(cred(), decodedPath, configuration.getApplication().getGoogle().getParentDrivePath());
			...
			throw new WebApplicationException(String.format("Could not get file: %s!", EX.getMessage()), Response.Status.BAD_GATEWAY);
		}
		return result;
	}
